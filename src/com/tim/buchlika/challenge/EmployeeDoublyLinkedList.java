...
public class EmployeeDoublyLinkedList {
    ...
    public boolean addBefore(Employee newEmployee, Employee existingEmployee) {
        ...
        while(!(current.getEmployee().getId()==existingEmployee.getId()))
        {
        	current=current.getNext();
            System.out.println(current.getEmployee().getId());
        }
        EmployeeNode pre=current.getPrevious();
        if(pre==null) {
        	newEmp.setNext(current);
        	head=newEmp;
        }
        else {
        System.out.println(pre.getEmployee().getFirstName());
        pre.setNext(newEmp);
        newEmp.setPrevious(pre);
        newEmp.setNext(current);
